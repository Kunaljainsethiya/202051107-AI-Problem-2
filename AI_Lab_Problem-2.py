...
def get_transition_prob(state, action):
    ...
    requests_normal = np.sum(poisson.rvs(
        0.1, size=len(pop_age_groups)) * pop_age_groups)
    requests_covid = np.sum(poisson.rvs(
        0.05, size=len(pop_age_groups)) * pop_age_groups)
    ...
    new_beds_normal = max(min(beds_normal + requests_normal -
                          discharges_normal - action, beds_normal_max), 0)
    new_beds_covid = max(min(beds_covid + requests_covid -
                         discharges_covid, beds_normal_max), 0)
    new_pop_age_groups = pop_age_groups + \
        norm.rvs(scale=1, size=len(pop_age_groups))
            ...
            for beds_normal in range(beds_normal_max+1):
                for week in range(4):
                    ...
                    for action in beds_convert_range:
                        next_state, reward = get_transition_prob(
                            (pop_age_groups, pop_total, beds_normal, beds_covid_max), action)
                        next_age_group = np.clip(np.sum(
                            next_state[0] * pop_age_groups / next_state[1]), 0, len(pop_age_groups)+1)
                        next_beds_normal = next_state[2]
                        next_week = week + 1 if week < 3 else 0
                        action_values.append(
                            reward + discount_factor * V[next_age_group, next_beds_normal, next_week])
    ...
    for beds_normal in range(beds_normal_max+1):
        for week in range(4):
            ...
            for action in beds_convert_range:
                ...
                action_values.append(
                    reward + discount_factor * V[next_age_group, next_beds_normal, next_week])
            policy[age_group, beds_normal,
                   week] = beds_convert_range[np.argmax(action_values)]
...
for week in range(4):
    ...
    for age_group in range(pop_age_groups.size+2):
        for beds_normal in range(beds_normal_max+1):
            print(
                f"{age_group}\t\t{beds_normal}\t\t{policy[age_group, beds_normal, week]}")
